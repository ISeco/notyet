...
| a step in `references/setup.md` that produces a contract section, **or** that section in `references/templates.md` | **the other one.** They are two halves of one instruction — the step says to write it, the template says what it looks like — and neither is complete alone, so a change to one silently leaves the other describing a thing that no longer matches. If the piece also carries a status line, its bullet in `README.md`'s "Where the evidence stops" too. This row exists because Step 4b and the `Shape of new code` section were written together and nothing in this table connected them |
...
**Where drift lands here:** five places have to agree with the files, and nothing automatic checks any of them — `SKILL.md`'s References section, its routing paragraph, the README's "What's in the box" table, the README's "Where the evidence stops" list, and the setup-step ↔ template pairs. Every mistake this repository has made so far was one of them disagreeing with reality.

**This table does not name itself, and that has now cost twice** — the version row undercounting its own fields, and the setup ↔ template coupling going unlisted the day it was created. Both were caught by reading while applying the rule, not by the rule. Treat "did this change create a new pair of files that must agree?" as the implicit last row.
